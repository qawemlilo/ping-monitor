...
function Monitor (opts, state) {
  ...
  this.state = {
    created_at: state ? state.created_at : null,
    isUp:  state ? state.isUp : false,
    host: '',
    port: null,
    totalRequests: state ? state.totalRequests : 0,
    lastDownTime:  state ? state.lastDownTime : null,
    lastRequest:  state ? state.lastRequest : null,
    interval: 15
  };
...
Monitor.prototype.init = function (opts, state) {
  ...
  this.state.host = website || address;
  this.state.port = port;
  this.state.interval = interval * ONE_MINUTE;

  if (!website && !address) {
      return this.emit('error', new Error('You did not specify a website to monitor'));
      this.state.isUp = false;
      this.state.lastDownTime = Date.now();
  }
...
Monitor.prototype.stop = function () {
  ...
  this.emit('stop', host, this.state);
}
...
Monitor.prototype.response = function (statusCode, data) {
  ...
  if (statusCode === 200) {
    this.emit('up', responseData, this.state);
  }
  else {
    this.emit('down', responseData, this.state);
  }
}
